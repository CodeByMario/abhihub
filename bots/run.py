...
Usage:
  python bots/run.py --list              # list all bots + roles
  python bots/run.py --dry-run           # run all bots, no network (deterministic)
  python bots/run.py --bot engagement    # run a single role bot
  python bots/run.py --cycle             # full company cycle via CEO bot
"""
import argparse
import importlib
import os
...
# Make `bots/` importable + add repo root so config can find .env.
...
def list_bots():
    ...
    print("\nRun: python bots/run.py --dry-run | --bot <name> | --cycle")
...
def main():
    ...
    if args.bot:
        ...
        print(f"   report -> bots/reports/{rep.bot_name}.md")
        return
    ...
    for rec in rep.recommendations:
        print("  - " + rec)
    print(f"\nConsolidated report -> bots/reports/company_consolidated.json")
